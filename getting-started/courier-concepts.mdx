---
title: "Courier Concepts"
description: "Terms and concepts you will commonly see when you use Courier's notification platform."
---
...
**Keep reading:** [Brands](../platform/content/brands/how-to-use-brands-to-customize)
...
Courier's [channel rules](../platform/sending/channel-priority) allow you to prioritize which Integrations are used to send your Notifications. Selected Integrations are arranged in a top-to-bottom hierarchy.

## Content Blocks

[Content blocks](../platform/content/notification-designer/notification-designer-overview/#reusable-drag-and-drop-content) are pieces of content that you add to your notification in Couriers drag and drop [Notification Designer](../platform/content/notification-designer/notification-designer-overview). Content blocks are responsive and reusable across channels.

## Conditions

The [Conditions](../platform/sending/channel-settings#conditions) tab in the [Notification](../platform/content/notification-settings/general-settings), [Channel](../platform/sending/channel-settings) or [Block](../platform/content/content-blocks/content-block-basics#block-settings) settings lets you apply conditionals to define when to send a notification, which channel the notification is routed to (even which integration) and which blocks to include.

## Device Token

A device token (push token) is a unique key for the app-device combination issued by the Apple or Google push notification gateways. It allows gateways and push notification providers to route messages and ensure the notification is delivered only to the unique app-device combination for which it is intended. Courier's [Token Management API](/reference/token-management) is used to store and track the status of user device tokens.

## Elemental

[Courier Elemental](../platform/content/elemental/intro) is a powerful JSON based syntax for describing the content of your notification in a way that will work for email, push, chat, or any channel you use to notify users. With Courier Elemental you can customize the look, language, and structure of your notification based on locale, channel, and other more advanced custom logic.

## Events

An event is a unique identifier that can be mapped to an individual Notification. These events can be defined by you, i.e. `new-user` or by an incoming [event through Segment](../tutorials/how-to-send-notifications-with-segment).

## Handlebars

Handlebars is a simple email templating language that generates HTML. The Courier Notification Designer lets you insert Template Blocks that use Handlebars or switch to the [Handlebars editor](../platform/content/notification-designer/handlebars-designer) and build your entire template in HTML.

## List

A `List` is a static group users defined by you via the [List API](../reference/lists/intro) or the [Users page](https://app.courier.com/users) in the Courier Web App by subscribing user profile(s) to the list.
...
Notifications are the messages you send through channels via the delivery systems you integrate with Courier. Each notification has a unique identifier known as the Event ID which you can find in the [notification settings](../platform/content/notification-designer/notification-designer-overview).

**Keep reading:** [What is Courier?](what-is-courier)
...
**Keep reading:** [Notification Design Studio.](../platform/content/notification-designer/notification-designer-overview)

## Override

Overrides can be used to change the request body that Courier sends to a provider. Overrides are useful when a field is not yet supported by Courier, or you would like to override the value that Courier generates. Here's an [Override example using SendGrid](../platform/channels/email/sendgrid#override).

## Profile

A [Profile](../reference/profiles/intro) is a JSON object that includes any key-value pairs required by our Integrations. You can find the required keys for any Integration on its "Integrations" page. See the [Integration Introduction](../platform/channels/intro) for details.
...
**Read more:** [White-labeling](../platform/content/brands/how-to-use-brands-to-customize)
